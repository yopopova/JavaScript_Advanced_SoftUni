class CarDealership {
    ...
    sellCar(model, desiredMileage) {
        ...
        if (this.availableCars.some(x => x.model === model)) {
            ...
            if (currCar.mileage > desiredMileage) {
                ...
                return `${model} was sold for ${soldCarPrice.toFixed(2)}$`; // newSoldCar.soldPrice
    ...
    currentCar() {
        if (this.availableCars.length === 0) {
            ...
            this.availableCars.forEach(x => {
                ...
                result.push(`---${x.model} - ${x.horsepower} HP - ${miles.toFixed(2)} km - ${carPrice.toFixed(2)}$`); // We print each one
            });
    ...
    salesReport(criteria) {
        ...
        this.soldCars.forEach(x => {
            let carPrice = x.price;
            result.push(`---${x.model} - ${x.horsepower} HP - ${carPrice.toFixed(2)}$`); // .toFixed(2)
        });
